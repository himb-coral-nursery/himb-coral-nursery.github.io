---
layout: page
title: 
subtitle: 
permalink: /research/
---

<!-- SECTION 1: COLLABORATIONS -->
<div class="section-collaborations" markdown="1">
<a id="collaborations"></a>
<h1 class="page-title">Collaborations</h1>

<div class="full-width-section collab-list" markdown="1">

**[UZELA: The Underwater Zooplankton Enhancement Light Array](https://www.researchgate.net/publication/388756450_Underwater_Zooplankton_Enhancement_Light_Array_UZELA_A_technology_solution_to_enhance_zooplankton_abundance_and_coral_feeding_in_bleached_and_non-bleached_corals)** - lead by [Dr. Andrea Grottoli (The Ohio State University)](https://earthsciences.osu.edu/people/grottoli.1)

---

**[Rapid Resilient Reefs for Coastal Defense (R3D)](https://arl.hawaii.edu/core-competencies/ocean-science/rapid-resilient-reefs-for-coastal-defense-r3d/)** - Applied Research Lab, Madin Lab, Coral Resilience Lab, Huang Lab (UH Manoa), Wangpraseurt Lab & Thode Lab (UCSD), Voss Lab (FAU)

</div>

</div>

<!-- SECTION 2: STUDENT PROJECTS -->
<div class="section-students" markdown="1">
<a id="students"></a>
<h1 class="page-title">Student Projects</h1>

<div class="full-width-section" markdown="1">

* Madeleine Hardt: [Project Title](#)
* Hugo Ducret: [Shading Implications for *Montipora capitata*](https://www.researchgate.net/publication/385865085_Shading_does_not_lower_thermal_tolerance_in_the_coral_Montipora_capitata_implications_for_conservation_intervention)
* Tristan Permentier: [Project Title](#)
* Garrett Fundakowski: [Project Title](#)
* Jasmine Awaya: [Project Title](#)
* Tyra Arends: [Project Title](#)
* Addison Trainer: [Project Title](#)

</div>

</div>

<!-- SECTION 3: MONITORING -->
<div class="section-monitoring" markdown="1">
<a id="monitoring"></a>
<h1 class="page-title">Long-term Monitoring</h1>

<div class="full-width-section" markdown="1">

The nursery is photographed bimonthly to generate 3D models and orthomosaics of each rack. This allows us to track coral health over time, measure growth rates, and identify bleaching or disease events. When combined with environmental data, researchers can select corals with known stress responses to better predict how reefs will respond to future climate events.

</div>

</div>
